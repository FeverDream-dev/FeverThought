...
impl LspServerConfig {
    pub fn builtin_configs() -> HashMap<String, Self> {
        ...
        configs.insert("typescript".into(), Self {
        ...
        configs.insert("python".into(), Self {
...
impl LspHost {
    ...
    pub fn config_for_extension(&self, ext: &str) -> Option<&LspServerConfig> {
        self.configs.values().find(|c| c.extensions.iter().any(|e| e == ext))
    }
    ...
    pub fn list_servers(&self) -> Vec<(ServerId, String)> {
        self.servers.read().iter().map(|(id, c)| (id.clone(), c.language_id().to_string())).collect()
    }
}
